...
> **Repository note (May 2026):** `speakez-xantham` is the Fidelity.CloudEdge-side working fork of upstream [`shayanhabibi/Xantham`](https://github.com/shayanhabibi/Xantham), serving as a staging area for fixes en route upstream (see §9.6.1 for the in-flight branch tracking). The durable home for Xantham itself is upstream; the speakez fork is retired once its outstanding patches merge. Forward-looking design discussion for the binding pipeline — JSIR integration, Transpose, two-layer analysis ([10_jsir_strategic_assessment.md §4.2](10_jsir_strategic_assessment.md)) — lives in this `Fidelity.CloudEdge/docs/` tree, not in the speakez fork's `docs/`.
