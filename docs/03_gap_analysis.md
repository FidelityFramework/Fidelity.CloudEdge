...
- **The official Cloudflare TypeScript API client** (`cloudflare` unscoped): a complementary control-plane binding alongside the Hawaii-generated Management tier, for surfaces where the TS client's ergonomics beat the autogenerated OpenAPI shape.
...
| `cloudflare` (unscoped) | `cloudflare/cloudflare-typescript` (npm) | 📋 Planned (post-0.3.0) | Official Cloudflare TypeScript API client for control-plane operations; complement to the Hawaii Management tier where TS ergonomics beat OpenAPI autogen |
