...
Following Cloudflare's Agents Week rollout (March–April 2026) and the subsequent monorepo build-out through mid-May 2026, the binding surface that Fidelity.CloudEdge needs to represent has expanded substantially beyond OpenAPI Management services and `@cloudflare/workers-types`. The Cloudflare intelligent-edge npm suite now constitutes the substantive AI/agentic scope of the project, and its coverage is core to "what Fidelity.CloudEdge is" — not a future addition:

- The **`cloudflare/agents` monorepo family**: `agents` (the renamed `agents-sdk`), `@cloudflare/ai-chat` (persistent chat + resumable streaming), `@cloudflare/voice` (STT/TTS/VAD/SFU), `@cloudflare/think` (opinionated agentic loop), `@cloudflare/codemode` (LLM-generated code-as-tool-calls), `@cloudflare/shell` (sandboxed JS + virtual FS), and `@cloudflare/worker-bundler` (runtime Worker bundling for the Worker Loader binding). All eight are published with SLSA v1 provenance from `cloudflare/agents`.
- **Adjacent runtime SDKs in separate Cloudflare repos**: `@cloudflare/dynamic-workflows` (multi-tenant workflow dispatch), `@cloudflare/containers` (container-enabled DO helper), `@cloudflare/sandbox` (sandboxed command execution), and `@cloudflare/puppeteer` (Puppeteer-API-compatible control over the Workers Browser-Rendering binding — load-bearing for agentic info-gathering on the open web; the same capability class Cloudflare uses in their own site-crawler topology). Puppeteer's audit treatment is more conservative because its publisher hasn't enabled npm trusted-publishing yet, but its substantive role is on par with the rest of the suite.
- The **Workflows V2 runtime additions** to `@cloudflare/workers-types`.

The unscoped `cloudflare` npm TypeScript client is **not** in scope here. It covers the same REST endpoints Hawaii already binds via the Management tier (`Fidelity.CloudEdge.Management.*`), so carrying it as a duplicate xantham ingestion target would double the maintenance surface without delivering new capability. If TS-client-style ergonomics are ever wanted on top of the Management bindings, that's an additive hand-curated layer, not a parallel binding.

These are tracked as rows in the "Intelligent-Edge SDKs" coverage matrix below. `hono-agents` is **out of scope** — Fidelity provides its own F#-native web framework occupying the same conceptual slot as Hono, so the Hono-specific integration layer carries no value into the F# surface. The 0.3.0 and subsequent release scopes are determined by what closes the highest-priority gaps in this matrix; the supply-chain audit policy in [13_supply_chain_audit.md](13_supply_chain_audit.md) governs the per-package ingestion gates.
...
### Intelligent-Edge SDKs (Xantham-Generated)

These are the AI/agentic surface that Cloudflare has staked the platform on. They carry first-class Cloudflare value in npm packages outside `workers-types` and are core to representing Cloudflare's intelligent-edge primitive set in F#. Coverage of this row group is **on par with** workers-types coverage — not a secondary concern.

All Cloudflare-published direct dependencies in this set (except `@cloudflare/puppeteer`) qualify for the supply-chain embargo lift documented in [13_supply_chain_audit.md](13_supply_chain_audit.md) on the strength of SLSA v1 provenance attestations. The per-package source repo and provenance evidence are recorded in the policy table there.

| Package | Source | Status | Surface |
|---------|--------|--------|---------|
| `agents` (formerly `@cloudflare/agents` / `agents-sdk`) | `cloudflare/agents` monorepo | 📋 Planned (0.3.0 target) | `Agent<'Env, 'State>` base class, `Think<'Env>` chat-agent subclass, lifecycle hooks (`onConnect`, `onMessage`, `onRequest`, plus Think's chat hooks `configureSession`, `beforeTurn`, `beforeStep`, `beforeToolCall`, etc.), `@callable()` RPC decorator, `setState`/`initialState`, scheduling, MCP, email, workflows, x402, browser-agents |
| `@cloudflare/ai-chat` | `cloudflare/agents` (sibling) | 📋 Planned (post-0.3.0) | Persistent chat messages, resumable streaming, tool execution. React-coupled exports filtered at the `IgnorePathRender` layer — the project's reactivity model is signals/SolidJS, not React |
| `@cloudflare/voice` | `cloudflare/agents` (sibling) | 📋 Planned (post-0.3.0) | STT/TTS/VAD/SFU, `VoiceClient`, server mixins (`withVoice`, `withVoiceInput`). React hooks filtered |
| `@cloudflare/think` | `cloudflare/agents` (sibling) | 📋 Planned (post-0.3.0) | Opinionated agentic loop, stream resumption, `submitMessages()` durable programmatic submission, client tools, workspace tools |
| `@cloudflare/codemode` | `cloudflare/agents` (sibling) | 📋 Planned (post-0.3.0) | LLM-generated TypeScript that orchestrates tool calls inside a sandbox, replacing one-tool-call-at-a-time patterns |
| `@cloudflare/shell` | `cloudflare/agents` (sibling) | 📋 Planned (post-0.3.0) | Sandboxed JS execution, virtual filesystem (`Workspace`), structured file I/O, transactional pattern replacement, git ops via `isomorphic-git` |
| `@cloudflare/worker-bundler` | `cloudflare/agents` (sibling) | 📋 Planned (post-0.3.0) | Runtime Worker bundling for the Worker Loader binding (esbuild-wasm + sucrase + typescript@^6); heaviest transitive load of the sibling family |
| `@cloudflare/dynamic-workflows` | `cloudflare/dynamic-workflows` (npm; published 2026-04-30) | 📋 Planned (0.3.0 target) | `createDynamicWorkflowEntrypoint`, `wrapWorkflowBinding`, `dispatchWorkflow`, `DynamicWorkflowBinding` for multi-tenant workflow dispatch |
| `@cloudflare/containers` | `cloudflare/containers` (npm) | 📋 Planned (post-0.3.0) | Helper class for container-enabled Durable Objects — DO + Container composition. Zero runtime deps |
| `@cloudflare/sandbox` | `cloudflare/sandbox-sdk` (npm) | 📋 Planned (post-0.3.0) | Containerized command execution; depends on `@cloudflare/containers` |
| `@cloudflare/puppeteer` | `cloudflare/puppeteer` (npm) | 📋 Planned (post-0.3.0) | Puppeteer-API-compatible control over the Workers Browser-Rendering binding — fetch JS-driven pages, capture DOM snapshots, take screenshots, and extract content as tool calls from an agent. The same capability class Cloudflare uses in their site-crawler topology; load-bearing for agentic info-gathering on the open web. Audit treatment is more conservative (no current SLSA provenance — full 72h policy per [13](13_supply_chain_audit.md)) but the substantive role is unchanged |
| `cloudflare` (unscoped) | `cloudflare/cloudflare-typescript` (npm) | ✗ Out of scope | Cloudflare's official TS control-plane client. Same REST endpoints Hawaii already binds via `Fidelity.CloudEdge.Management.*`; carrying both would double maintenance for zero new capability |
| `hono-agents` | `cloudflare/agents` (sibling) | ✗ Out of scope | Fidelity provides its own F#-native web framework occupying the same conceptual slot as Hono; the Hono integration layer carries no value into the F# surface |
