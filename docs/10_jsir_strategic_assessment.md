...
**Forward observation — two-layer analysis (Pilot analogy).** A future Transpose enhancement could mirror Farscape's *Pilot* feature for C/C++ bindings, where Pilot analyzes both header files (declared surface) and the pre-compile AST (constructed reality) to choose a binding strategy. The TypeScript analog ingests `.d.ts` (declared types) *and* the SDK's compiled JavaScript (lifted into MLIR via JSIR's ingestion direction; see §1 "Round-trippable") to produce richer bindings. The second layer resolves cases where the declared surface is ambiguous: re-export reality across barrel files, default-vs-named export shape, refined return shapes where a declared union has stable per-branch shape in the implementation, and `package.json exports` conditional resolution where the `.d.ts` path can differ from the JS the runtime actually loads. JSIR's bi-directionality (designed for lifting, capable of emission) makes this architecturally available — the same dialect Composer uses to emit JavaScript can be used by Transpose to read it. For Xantham's current three-phase architecture, the analogous extension is a JS-implementation pass feeding additional facts into the existing JSON IR; no rewrite, an additional input source.

**Security-analysis natural fit.** JSIR's primary internal-Google use case — JavaScript deobfuscation and malicious-code detection (§1) — is directly applicable to the same lift pass. The analyzer can flag suspicious patterns in an SDK's JS (network beacons, exfiltration shapes, install-script behavior, dynamically-constructed `eval`/`Function` calls) at binding-generation time, complementing the install-time per-publisher lift policy in [13_supply_chain_audit.md](13_supply_chain_audit.md). Install-time audit catches what the registry serves *now*; a JSIR-lift inspection catches what the binding-host *actually sees* during generation, and the analytical result persists in the IR rather than only at the install moment. Recorded as observation, not commitment — but a natural target as JSIR's analysis surface stabilizes upstream.
