...
`Fidelity.CloudEdge.DurableObjects` is the low-level binding layer to Cloudflare's Durable Object runtime types from `@cloudflare/workers-types`. It exposes platform types directly: `DurableObjectState`, `DurableObjectId`, `DurableObjectNamespace`, WebSocket handlers, transactional storage. The current generation tooling is Glutinum (legacy) and is migrating to Xantham per [00 Decision 7](00_architecture_decisions.md); the binding generator change does not affect the API surface application code consumes. Most application code does not touch this layer directly; it is consumed transitively by the Agents bindings.

`Fidelity.CloudEdge.Agents` provides F# bindings on Cloudflare's `agents-sdk` npm package (formerly `@cloudflare/agents`). `Agent<'Env, 'State>` and `Think<'Env, 'State>` are F# classes that wrap Cloudflare's TypeScript Agent and Think classes. The 0.3.0 release ships these as hand-curated `Types.fs` files because Glutinum crashed on the agents-sdk surface; per [00 Decision 7](00_architecture_decisions.md), they are migrating to Xantham-generated output as the standard runtime binding pipeline. Applications writing in the **Cloudflare-native authoring style** inherit from these directly and work with `SetState`, `InitialState`, `[<Callable>]` methods, and the framework's lifecycle conventions.
