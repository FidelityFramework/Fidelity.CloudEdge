...
let fableProjects = [
    // Core
    "Core/CloudEdge.Core", "Core/CloudEdge.Core"
    // Runtime (Worker.Context first as it has base types)
    "Runtime/CloudEdge.Worker.Context", "Runtime/CloudEdge.Worker.Context"
    "Runtime/CloudEdge.AI", "Runtime/CloudEdge.AI"
    "Runtime/CloudEdge.D1", "Runtime/CloudEdge.D1"
    "Runtime/CloudEdge.DurableObjects", "Runtime/CloudEdge.DurableObjects"
    "Runtime/CloudEdge.Hyperdrive", "Runtime/CloudEdge.Hyperdrive"
    "Runtime/CloudEdge.KV", "Runtime/CloudEdge.KV"
    "Runtime/CloudEdge.Queues", "Runtime/CloudEdge.Queues"
    "Runtime/CloudEdge.R2", "Runtime/CloudEdge.R2"
    "Runtime/CloudEdge.Vectorize", "Runtime/CloudEdge.Vectorize"
]
...
/// Generates the unified Fidelity.CloudEdge.Runtime.fsproj for Fable
let generateFableFsproj () =
    printfn "Generating Fidelity.CloudEdge.Runtime.fsproj..."
    ...
    let fsprojContent =
        String.concat "\n" [
            ...
            """"""
            """    This is a Fable-specific project file that consolidates all Fidelity.CloudEdge"""
    ...
    let outputPath = Path.Combine(objDir, "Fidelity.CloudEdge.Runtime.fsproj")
    File.WriteAllText(outputPath, fsprojContent)
    printfn "Generated: %s" outputPath

pipeline "Fidelity.CloudEdge NuGet" {
    description "Build and pack Fidelity.CloudEdge.Runtime and Fidelity.CloudEdge.Management NuGet packages"

    stage "Build .sln" {
        workingDir root
        run "dotnet build Fidelity.CloudEdge.sln -c Release"
    }
